...
from lib.dataset import DoubleMNIST, DoubleSameMNIST
import argparse
...
def parse_args():
    ...
    parser.add_argument('-p', help="padding mode",
                        choices=['zeros', 'border'], default="border")
    ...
    same: 2 same image in training
    ''',
                        choices=['mnist', 'double', 'one', 'same'], default="mnist")    
    return parser
...
if __name__ == '__main__':
    ...
    if args.e == 'mnist':
        ...
        train_data = DoubleSameMNIST(train=True, transform=transforms.Compose([
            RandomRTS(degree=(-args.r, args.r),
                      translate=(-args.t, args.t),
                      scale=(1-args.s,1+args.s)),
            wrap(transforms.ToTensor()),
    ...
    t = Trainer(model, optimizer, use_cuda=use_cuda, twoImage=(args.e=='same'))
    ...
    for epoch in range(1, 10+1):
        ...
        loc = os.path.join(loc,
                           "%s_R%d_T%d_S%d.cpt" % (args.m, args.r,
                                                   args.t*100, args.s*100))
